import { photographerComponents } from '../factories/photographer.js'
...
async function displayData (photographer, images) {
  ...
  prevModalMedia.addEventListener('click', () => {
    ...
    const lastMediaIndex = images[0].findIndex(media => media.id === parseInt(lastImgId))
    let newMediaIndex
    if (lastMediaIndex === 0) {
  ...
  const closeContactModalBtn = document.querySelector('#close_contact_modal')
